...
| 8   | Combat + projectiles          | health, weapons, damage, chase/leash, death, explicit multi-ring group engagement formation                                                                                                               | ✅ done (armor table + abilities: pending)             |
...
| 21  | Maps and scenarios            | load editor maps into matches, deterministic triggers, objectives and scripted events, including phase-gated Siege Line completion and a tested browser Worker initialization boundary                     | in progress (all five campaign operations implemented) |
...
| 27  | Commander achievements        | versioned commander profiles, isolated progress, six authoritative trophies, per-operation decoration provenance and Battle Reports for conventional matches plus tutorial-complete operations             | ✅ done                                                |
| 28  | Battlefield medicine          | unarmed barracks Medic, deterministic infantry-only auto-assist and treatment animation; manual treatment and condition-specific actions are intentionally out of scope                                      | ✅ done                                                |
...
| 30  | Battlefield life and cover    | seeded rocks and inspectable war remains, directional infantry cover from authored blocked cells, presentation-only animal runs and military flyovers, plus a deterministic commander-profile chicken event | ✅ done                                                |
| 31  | Infantry condition and morale | deterministic per-infantry condition states and Medic stabilization actions                                                                                                                               | deferred by product decision                           |
